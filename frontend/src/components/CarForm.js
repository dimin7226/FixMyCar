...
const CarForm = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get('http://localhost:8080/home/customers');
  ...
  const fetchCar = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/home/cars/${id}`);
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`http://localhost:8080/home/cars/${id}`, dataToSend);
        message.success('Автомобиль успешно обновлен');
      } else {
        await axios.post('http://localhost:8080/home/cars', dataToSend);
